...
const differentiators = [
  { label: 'Estructura modular', body: 'Cada equipo se construye según la necesidad del proyecto. Tu presupuesto se invierte en talento, producción y distribución efectiva, no en estructuras fijas.' },
  { label: 'Low overhead, high output', body: 'Sin capas innecesarias de management. El equipo que hace tu brief es el que ejecuta tu proyecto. Sin brief que se pierde entre intermediarios.' },
    ...
    <div>
        ...
        <div className="max-w-5xl mx-auto">
          <InViewSection>
            ...
            <motion.div variants={fadeUp} className="flex flex-col gap-4 text-sm text-neutral-500 leading-relaxed max-w-[56ch]">
              <p>Conectamos creatividad con industria para hacer crecer la economía local. No como slogan, sino como modelo operativo.</p>
              <p>Las marcas que sobreviven no son las más grandes ni las que gastan más en publicidad. Son las que tienen una perspectiva clara, ejecutan rápido y miden todo.</p>
            </motion.div>
          </InViewSection>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <InViewSection className="grid grid-cols-1 md:grid-cols-2 gap-px bg-neutral-200">
            {differentiators.map((d) => (
              <motion.div key={d.label} variants={fadeUp} className="bg-white p-8">
      ...
      <section className="px-6 md:px-12 py-[104px] border-b border-neutral-200">
        <div className="max-w-5xl mx-auto">
          ...
          <InViewSection className="grid grid-cols-2 md:grid-cols-5 gap-6">
            {leadership.map((p) => (
      ...
      <section className="px-6 md:px-12 py-[104px] border-b border-neutral-200 bg-neutral-50">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-sm text-neutral-500 mb-10 max-w-[48ch]">Alianzas estratégicas que amplían nuestra capacidad sin inflar nuestra estructura.</p>
          <InViewSection className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-px bg-neutral-200">
            {partners.map((p) => (
              <motion.div key={p.name} variants={fadeUp} className="bg-white p-6">
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-end md:justify-between gap-8">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <Link href="/lofi/trabajos" className="inline-flex items-center gap-2 text-xs font-medium px-4 py-2 rounded-full border border-neutral-300 text-neutral-600 hover:border-neutral-600 hover:text-neutral-900 transition-colors">
              Ver trabajos <ArrowRight size={12} className="shrink-0" />
            </Link>
          </div>
        </div>
      </section>
